...
- **Always Up to Date** - Servant data is synchronized automatically from both JP and NA servers using GitHub Actions and the Atlas Academy API.
- **Servant Archive** - Browse the complete database of servants, complete with lazy-loaded profile images and stats.
- **JP / NA Server Toggle** - Instantly switch between the Japanese and North American server rosters. Your daily servant stays synced with your chosen region.
- **Smart Search** - Look up servants by name or alias without worrying about accents or diacritics.
- **Mobile Responsive** - Fully scrollable grids and a mobile-optimized navigation menu work seamlessly on any screen size.
